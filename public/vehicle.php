    ...
    <div class="col-md">
        ...
        <div class="card shadow-sm p-3 mb-2 bg-white rounded">
            <div class="card-body">
                ...
                <small class="card-title text-muted">Vehicle Preview</small>
            ...
            <div class="row mt-2">
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <th scope="row">{{view.vehicle_code}}</th>
                        <td>{{view.vehicle_type}}</td>
                        <td>
                            <i class='bx bx-pencil' style="cursor:pointer" v-on:click="edit_list(index,view.vehicle_code)"></i>
                            <i class='bx bx-trash-alt' style="cursor:pointer" v-on:click="delete_list(index,view.vehicle_code)"></i>
                        </td>
                        </tr>
...
var license = new Vue({
    ...
    methods:{
        ...
        async list () {
            const fetch_list = await axios.get('<?php echo SITE_URL;?>/api/vehicle/viewvehicle.php');
        ...
        async create__into (){
            ...
            const create_pending = await axios.get('<?php echo SITE_URL;?>/api/vehicle/createvehicle.php?value='+`${this.value}`);
            ...
            switch(create_pending.status){
                ...
                case 200:
                    ...
                    if(stats.msg == "success"){
                        ...
                        this.last = this.last == null ? last.vehicle_code+1 : this.last+1;
                        this.preview.push({vehicle_code : this.last, vehicle_type: this.value.toLowerCase()});
        ...
        async Update__into (){
            ...
            const update_pending = await axios.get('<?php echo SITE_URL;?>/api/vehicle/updatevehicle.php?value='+`${this.value}`+'&id='+`${this.toupdatekey}`);
            ...
            switch(update_pending.status){
                ...
                case 200:
                    ...
                    if(stats.msg == "success"){
                        ...
                        update.vehicle_type = this.value.toLowerCase();
        ...
        async Delete__into (id){
            ...
            const delete_pending = await axios.get('<?php echo SITE_URL;?>/api/vehicle/deletevehicle.php?value='+id);
    ...
    computed:{
        ...
        check_if_exist : function(){
            var Exist = this.preview.findIndex( s => s.vehicle_type == this.value.toLowerCase())
        ...
        searchlist() {
            if(this.search != ''){
                return this.preview.filter(list => {
                    ...
                    return list.vehicle_type.toLowerCase().includes(this.search.toLowerCase())
                })
            }else{
